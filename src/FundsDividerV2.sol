...
contract FundsDividerV2 {
    ...
    /// @notice 手续费百分比（可配置，基数为 10000）
    uint256 public feePercentage = 300; // 默认 3%
    ...
    /// @notice 手续费百分比更新事件
    /// @param oldFeePercentage 旧的手续费百分比
    ...
    constructor(
        address _admin,
        address _feeAddress
    ) onlyValidAddress(_admin) onlyValidAddress(_feeAddress) {
        admin = _admin;
    ...
    function updateAdmin(
        address _newAdmin
    ) external onlyAdmin onlyValidAddress(_newAdmin) {
    ...
    ) external onlyAdmin onlyValidAddress(_newFeeAddress) {
    ...
    /// @notice 更新手续费百分比
    /// @param _newFeePercentage 新的手续费百分比（基数为10000，不能超过100%）
    ...
    function setBeneficiaryList(
        address destAddress,
        Beneficiary[] calldata beneficiaries
    ) external onlyAdmin onlyValidAddress(destAddress) {
        if (beneficiaries.length == 0) revert EmptyBeneficiaryList();

        // 验证百分比总和不超过 100%
        uint256 totalPercentage = 0;
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            if (beneficiaries[i].beneficiary == address(0))
        ...
        onlyValidAmount(msg.value)
    {
        ...
        if (hasBeneficiaryList[destAddress]) {
            // 使用受益人列表进行分配
            distributions = _distributeWithBeneficiaryList(
                destAddress,
                totalAmount,
                true
            );
            ...
            // 使用默认手续费分配
            distributions = _distributeWithDefaultFee(
                destAddress,
                totalAmount,
                true
            );
    ...
    function _distributeWithBeneficiaryList(
        ...
        // 2. 从剩余金额中按比例分配给受益人
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            uint256 beneficiaryAmount = (remainingAfterFee *
                beneficiaries[i].percentage) / PERCENTAGE_BASE;
            distributions[index] = Distribution({
            ...
            if (isNative) {
                (bool success, ) = beneficiaries[i].beneficiary.call{
        ...
        if (beneficiaryTotalPercentage < PERCENTAGE_BASE) {
            uint256 finalRemainingAmount = remainingAfterFee -
                distributedFromRemaining;
            distributions[index] = Distribution({
